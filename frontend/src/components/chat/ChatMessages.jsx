  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-2 flex flex-col gap-3 min-h-0">
      {messages.map((msg) => (
        ...
        >
          <div
            className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed shadow-sm transition-all duration-200 animate-[slideUpFadeIn_0.25s_ease-out] ${
              msg.sender === 'user'
                ? 'bg-[var(--bitsy-user-bubble)] text-white rounded-br-md'
                : 'bg-[var(--bitsy-bot-bubble)] text-gray-800 border border-gray-100 rounded-bl-md'
            }`}
          >
            {msg.text}
          </div>
      ...
      {isLoading && (
        <div className="flex justify-start">
          <div className="bg-white border border-gray-100 px-4 py-3 rounded-2xl rounded-bl-md shadow-sm flex gap-1.5 items-center">
            <span className="typing-dot w-2 h-2 rounded-full bg-[var(--bitsy-purple)]" />
            <span className="typing-dot w-2 h-2 rounded-full bg-[var(--bitsy-purple)]" />
            <span className="typing-dot w-2 h-2 rounded-full bg-[var(--bitsy-purple)]" />
          </div>
        </div>
